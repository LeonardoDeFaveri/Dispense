...
\begin{graph}
    ...
    \tikzset{
        cell/.style={draw, rectangle, minimum size=10mm}
    }
    \def\numbers{13 14 15 12 20 27 29 30 21 25 28}
    \readarray\numbers\num[1, 11]

    \foreach \i in {1,...,4} {
        \node[cell, fill=pink] (\i) at (\i, 0) {$\num[1,\i]$};
    }
    \node[label=below:{\shortstack[l]{$A[lo\dots j-1]$\\$A[i]<A[j]$}}] (l1) at (1) [shift={(7mm, -3.1mm)}] {};
    \node[cell, fill=leaf, label=below:{\shortstack{$j$\\$p=A[j]$}}] (5) at (5, 0) {$\num[1,5]$};
    \foreach \i in {6,...,11} {
        \node[cell, fill=yellow] (\i) at (\i, 0) {$\num[1,\i]$};
    }
    \node[label=below:{\shortstack[r]{$A[j+1\dots hi]$\\$A[i]>A[j]$}}] (l2) at (11) [shift={(-7mm, -3.1mm)}] {};
\end{graph}
...
\begin{graph}
    ...
    \def\numbers{12 13 14 15 20 21 25 27 28 29 30}
    \readarray\numbers\num[1, 11]

    \foreach \i in {1,...,4} {
        \node[cell, fill=pink] (\i) at (\i, 0) {$\num[1,\i]$};
    }
    \node[cell, fill=leaf, label=below:$j$] (5) at (5, 0) {$\num[1,5]$};
    \foreach \i in {6,...,11} {
        \node[cell, fill=yellow] (\i) at (\i, 0) {$\num[1,\i]$};
    }
\end{graph}
