...
\begin{proof}[Dimostrazione]
    ...
    \frac{C}{w[i]}, 1\right)\leq1$. Allora, possiamo costruire una nuova soluzione
...
\end{proof}
\footnotetext{Siccome $T'$ è ottenuto da $T$ scambiando le codifiche di $x$ e
$a$, vale che $d_T(x)=d_{T'}(a)$ e $d_T(a)=d_{T'}(x)$}
...
\emph{arco} che collega un \emph{nodo} dell'\emph{albero} a un \emph{nodo} in
$Q$ o $+\infty$ se tale \emph{arco} non esiste. Inoltre, implementando
l'\emph{albero} usando il \emph{vettore dei padri}, ogni \emph{nodo} $v$
mantiene un puntatore al proprio \emph{padre} $p[v]$ e quindi l'insieme $A$
è mantenuto implicitamente:
