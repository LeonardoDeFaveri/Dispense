...
soluzione, siamo quindi costretti a rinunciare a qualcosa, e questo significa
...
Iniziamo rivedendo la soluzione con \emph{backtracking}.
...
non è possibile definire alcuni sottoinsieme diverso da quello vuoto.
...
dover inizializzare l'intera tabella, usiamo un \emph{dizionario}.
...
che $k$ non è la dimensione dell'input, è parte dell'input. In particolare, $k$
viene rappresentato da $t=\lceil\log k\rceil$ cifre binarie, quindi $O(nk)$ può
anche essere scritto come $O(n2^t)$ che è una \emph{complessità esponenziale}.

\subsection[Problemi fortemente e debolmente NP-completi]
{Problemi fortemente e debolmente $\mathbb{NP}$-completi}
Per continuare la discussione sul tema della \emph{pseudo-polinomialità} facciamo
una digressione sui \emph{problemi fortemente e debolmente $\mathbb{NP}$-completi}.

\begin{definition}[Dimensioni del problema]
    Dati un problem decisionale $R$ e una istanza $I$, chiamiamo $d$ la lunghezza
...
\begin{note}
    ...
    i valori presenti e, siccome SUBSET-SUM è \emph{debolmente $\mathbb{NP}$-completo},
    lo è anche PARTITION.
\end{note}
...
\begin{definition}[Algoritmo di approssimazione]
    Se è possibile dimostrare che un limite superiore o inferiore al rapporto
    tra la soluzione trovata e la soluzione ottima, allora l'algoritmo usato
    per ricavare la soluzione non ottima è detto algoritmo di approssimazione.
\end{definition}
...
\begin{note}
    ...
    caso, dimostrare il valore scelto sia rispettato dall'algoritmo è ciò che
...
\begin{proof}[Dimostrazione]
    Sia $N$ il numero di scatole usata dall'algoritmo \emph{first fit}. Il numero
    minimo di scatole utilizzabili è limitato inferiormente da:
...
\begin{note}
    ...
    approssimazione} per il problema del \emph{bin packaging}.
\end{note}
...
\begin{problem}[Commesso viaggiatore con disuguaglianze triangolari ($\bc{\Delta}$-TSP)]
    ...
    distanza percorsa complessiva sia minima
\end{problem}
\begin{note}
