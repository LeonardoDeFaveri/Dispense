...
dinamica} è un'altra tecnica che però si basa su un approccio molto simile.
...
\begin{proof}[Dimostrazione]
    ...
    così, $S'_{i-1}$ dovrebbe essere anche la soluzione di $P_i$, ma ciò sarebbe
    assurdo dato che abbiamo supposto che $S_i$ fosse una soluzione ottima per
    $P_i$.
...
\emph{\nameref{def:111}} possiamo fare le seguenti assunzioni:
\begin{itemize}
...
\begin{definition}[Parentesizzazione]
    ...
    parentesizzazioni $(P_{i,k}\cdot P_{k+1,j})$ altrimenti.
\end{definition}
...
\begin{proof}[Dimostrazione]
    ...
    \begin{itemize}
        ...
        \item Se supponessimo esistesse una \emph{parentesizzazione ottima}
        $P'[k+1\dots j]$ di $A[k+1\dots j]$ con costo inferiore a $P[k+1\dots j]$,
        allora $P[i\dots k]\cdot P'[k+1\dots j]$ sarebbe una
        \emph{parentesizzazione ottima} di $A[i\dots j]$ con costo inferiore a
        $P[i\dots j]$, generando un assurdo;
    \end{itemize}
