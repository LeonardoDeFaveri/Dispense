...
or deny resource usage to processes or group of processes. They consist of two
components: kernetl support and user-space tools that hadles the kernel control
mechanism.
...
structure. Up to now, the Linux kernel offers seven typed of \emph{namespaces}:

{
    ...
    \begin{longtable}{|l|l|l|}
        ...
        \hline
        PID & \texttt{CLONE\_NEWPID} & Process ID's\\
        \hline
...
kill others. To avoid that, \emph{PID namespaces} allow creating nested
...
network setup (i.e. loopback interface, generale interfaces, routing tables,
firewall rules, \dots). Once a \emph{network namespace} is created, we should
create an additional \q{virtual} network interface which spans multiple
...
\emph{namespace}, thus allowing traffic move between the two \emph{namespaces}.
...
\begin{itemize}
    ...
    \emph{namespace}, without giving it that access to process outside the
    \emph{namespace};
...
When it comes to portability, VMs are still preferable. So, \emph{cgroups} and
\emph{namespaces} need to be extended and made easier to use; \emph{Linux
containers} goes in this direction.

Containers provide a \emph{lightweigth virtualization} that allow processes and
...
So, \emph{Docker} aims to obtain a clean separation of environments, an easy
...
\emph{Docker} is not a virtualization engine bacause it leverages existing
...
system distribution (it needs to a Linux distro tho) and network setting.
...
\emph{LXC} that focus more on containers as lightweigth machines. Then,
\emph{Dockers} containers are portable across machines, while \emph{LXCc} need
to be rebuilt.

\subsection{Docker usage in general}
\emph{Docker} distinguish between images and containers. The first are
immutable templates for container that can be pushed and pulled from a registry.
The seconds, on the other hand, are instances of an image that be started,
stopped, etc. Thus, they maintain thery state within their file system.
...
containers running on the same \emph{Docker host} can communicte each other
...
to assign IP addresses. Instead, network operations are carried out by the
...
container, and configure the inside endpoint of the virtual interface with to
...
container and that each container containes all that files required to run.
Hence, \emph{Docker} must grant isolationa and portability.

\begin{note}
    Whereas containers need to hold all theri files, each container might
...
separate file system. In other words, it mounts multiple directories to a
...
\emph{Docker} allow developer to automatically create a container starting from
